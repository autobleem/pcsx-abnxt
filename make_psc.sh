...
#   ./make_psc.sh --clean    wipe the remote build dir first, a full rebuild (-k, the old spelling of
#                            keeping it, is still accepted)
#   PCSXAB_PSC_HOST=other-host ./make_psc.sh
set -e
cd "$(dirname "$0")"

HOST="${PCSXAB_PSC_HOST:-psc-build}"                       # a Host entry in ~/.ssh/config (see above)
REMOTE_DIR="${PCSXAB_PSC_DIR:-pcsx-abnxt}"                    # relative to the server user's home
