...
# repository (autobleem-develop's tools/repo_publish.sh pcsx <version> dist/packages/*):
#
#   dist/packages/pcsx-abnxt-<version>-psc.tar.gz         build_psc/dist    Autobleem/bin/emu/ on the stick
#   dist/packages/pcsx-abnxt-<version>-rpi-armhf.tar.gz   build_rpi/dist    Autobleem/bin/emu/ on a 32-bit Pi
#   dist/packages/pcsx-abnxt-<version>-rpi-arm64.tar.gz   build_rpi64/dist  ... on a 64-bit Pi
#   dist/packages/pcsx-abnxt-<version>-pcusb.tar.gz       build_pcusb/dist   AutoBleem's 32-bit PC USB stick
#   dist/packages/pcsx-abnxt-<version>-win64.zip          build_win_rel (or build_win)  the exe, its DLLs, the plugins, the skin
#   dist/packages/pcsx-abnxt-<version>.json               what is in them: version, commit, date, sha256 per file
#
...
# The version is `git describe` (r26-20-gf43567a0: upstream's tag, our commits past it, the commit).
#
...
pack_linux() { # pack_linux <dist dir> <platform>
    local dist="$1" plat="$2" name="pcsx-abnxt-$VERSION-$2.tar.gz"
...
pack_windows() {
    ...
    local name="pcsx-abnxt-$VERSION-win64.zip" stage src=build_win
...
echo "pcsx-abnxt $VERSION ($COMMIT, $DATE):"
...
{
    printf '{\n  "name": "pcsx-abnxt",\n  "version": "%s",\n  "commit": "%s",\n  "date": "%s",\n' "$VERSION" "$COMMIT" "$DATE"
    printf '  "note": "a development build of the next emulator - runs on the Raspberry Pi; the PlayStation Classic is untested",\n'
    printf '  "files": {\n'
    first=1
    for f in "${files[@]}"; do
        plat="${f%%:*}"; name="${f#*:}"
    ...
    printf '\n  }\n}\n'
} > "$OUT/pcsx-abnxt-$VERSION.json"
echo "  manifest: pcsx-abnxt-$VERSION.json"
ls -l "$OUT"
