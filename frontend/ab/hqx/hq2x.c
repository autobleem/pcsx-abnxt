...
/*
 * pcsx-abnxt: hq2x over RGB565, from grom358/hqx's hq2x.c (LGPL 2.1+, see above). The pattern
...
#define PIXEL00_0     *dp = HQX_PACK(w[5]);
...
#define PIXEL01_0     *(dp+1) = HQX_PACK(w[5]);
...
#define PIXEL10_0     *(dp+dpL) = HQX_PACK(w[5]);
...
#define PIXEL11_0     *(dp+dpL+1) = HQX_PACK(w[5]);
...
void hq2x_16_16(const uint16_t *sp, uint16_t *dp, unsigned int Xres, unsigned int srb, unsigned int drb, unsigned int Yres)
{
    unsigned int i, j;
    int  k;
