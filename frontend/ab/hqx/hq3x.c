...
/*
 * pcsx-abnxt: hq3x over RGB565, from grom358/hqx's hq3x.c (LGPL 2.1+, see above). The pattern
...
#define PIXEL12_C   *(dp+dpL+2) = HQX_PACK(w[5]);
...
#define PIXEL20_C   *(dp+dpL+dpL) = HQX_PACK(w[5]);
...
#define PIXEL21_C   *(dp+dpL+dpL+1) = HQX_PACK(w[5]);
...
#define PIXEL22_C   *(dp+dpL+dpL+2) = HQX_PACK(w[5]);

void hq3x_16_16(const uint16_t *sp, uint16_t *dp, unsigned int Xres, unsigned int srb, unsigned int drb, unsigned int Yres)
{
    unsigned int i, j;
    int  k;
