/*
 * Host layer for the Windows (MinGW / MSYS2 UCRT64) development build of pcsx-abnxt: what
 ...
 * states - not as a product; it runs the lightrec dynarec and the C-SIMD NEON GPU.
 *
...
/* linux/plat.c is built with NO_HOME_DIR on every AutoBleem target: the root is the executable's
 * directory (plugins/, skin/, and pcsx's own .pcsx/ relative to the working directory). The same here, so
 * a run on the PC lays its files out exactly as the console and the Pi do. */
...
struct dirent *readdir(DIR *dir)
{
	...
	/* d_type as <dirent.h> here defines it; d_ino carries the same kind in libpicofe/posix.h's numbering
	 * (DT_REG 0, DT_DIR 1, DT_LNK 2), because that header's MinGW branch rewrites d_type to d_ino for
	 * the file browser in libpicofe/menu.c - the one place the two disagree */
